...
| WF-C12-008 | P1 | PARTIAL | Backend/SRE | Add reconciliation jobs for claim/event/assessment/exception/approval/export invariants | [`reconciliation kernel`](../src/lib/workforce/reconciliation.ts) and [paged job](../src/lib/workforce/reconciliation-job.ts) check bounded claim/event/evidence/assessment/exception/approval/export chains, verify immutable approval hashes, compare-and-set a cursor only after clean pages and return identifier-free counts with `repair: NONE`; durable database adapter, schedule and staging exercise remain open |
