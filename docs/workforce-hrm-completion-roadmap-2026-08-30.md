...
| WF-C9-007 | P0 | PARTIAL | Mobile | Implement permission-aware action-time location capture and approved on-duty transition mode; stop at Finish/off-shift | [`Android foundation`](./workforce-c9-android-foundation-evidence-2026-08-30.md) and [`explicit action-policy contract`](./workforce-c9-action-time-location-contract-evidence-2026-08-31.md): foreground-only current-location primitive and policy/manifest parser exist but remain source-gated from capture; server action binding, reviewed fallback and physical background/permission/battery evidence remain open |
