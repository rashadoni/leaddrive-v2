...
| WF-C6-005 | P1 | PARTIAL | Web | Build exception queue with scope, risk, age, evidence completeness, employee response and next action | [`queue foundation`](./workforce-c6-exception-queue-foundation-evidence-2026-08-30.md) and [`scoped backend`](./workforce-c6-scoped-workbench-backend-evidence-2026-09-26.md): the raw-proof-free queue now filters metadata by immutable historical team/site grants before loading names and returns exact non-terminal actions through encrypted tokens; visible action UI, shared-lock terminal resolution and browser/disposable-DB evidence remain open |
