...
| WF-C12-008 | P1 | PARTIAL | Backend/SRE | Add reconciliation jobs for claim/event/assessment/exception/approval/export invariants | [`reconciliation kernel`](../src/lib/workforce/reconciliation.ts) checks bounded claim/event/evidence/assessment/exception/approval/export chains, verifies immutable approval hashes and returns only finite counts with `repair: NONE`; persistent cursor, database reader and scheduled staging exercise remain open |
