...
| WF-C5-012 | P1 | PARTIAL | HR/Security | Define lost/stolen phone, employee termination and admin compromise playbooks | [`Device and access recovery playbooks`](./workforce-device-access-recovery-playbooks-2026-09-13.md) define narrow, privacy-safe revoke/freeze/recovery sequences and measurable acceptance; staging/physical exercises remain NOT RUN |
...
| WF-C9-002 | P0 | PARTIAL | Mobile/Backend | Implement secure login/bootstrap with tenant module manifest, permissions, policy/config versions and forced-update response | [`C9 mobile bootstrap evidence`](./workforce-c9-mobile-bootstrap-evidence-2026-09-13.md): authenticated split module/permissions and effective attendance config already exist; the additive Android version contract is inert by default and fails closed once coherently configured; native consumption and signed-app evidence remain open |
...
| WF-C13-003 | P0 | PARTIAL | Backend | Version mobile request/response/evidence schemas and advertise support in bootstrap | Workday schemas 1-3, evidence v1 and cohort-bound protocol advertisement are pinned; a structured forced-upgrade response remains gated on the approved adoption/offline-drain threshold |
