...
| WF-C7-002 | P1 | PARTIAL | Backend | Implement granular scopes: employee self, team/site manager, scheduler, time approver, evidence reviewer, device admin, export custodian, retention/legal-hold officer, pilot/rollback operator and tenant admin | [`granular foundation`](./workforce-c7-granular-access-foundation-evidence-2026-08-30.md) and [`write fence`](./workforce-c7-grant-management-write-fence-evidence-2026-09-05.md): pure fail-closed contracts, a persisted resolver, additive empty ledger and replay-safe transaction writer now back MFA/session-only grant/revoke, bounded active inventory and tenant-local target search with serializable actor/target rechecks and shared fail-closed budgets; browser role-management UI, initial custody, tenant activation and disposable-DB/RLS evidence stay open |
