...
- [ ] C1.13 Route current REST operations through the same command services.
      ...
      rollup; stage rules, revenue transitions, loyalty, surveys). REST keeps
      its "drop unwritable fields" contract by filtering before the call.
...
| 2026-09-21 | Edit tasks and deals by voice | Code complete | Branch `codex/voice-update-task-deal`; targeted Vitest + 7 mutation checks; full suite matches baseline | `update_task` (title, description, priority, due date, assignee, status by meaning) and `update_deal` (name, amount, currency, close date, company, contact, assignee, notes). No deal stage/pipeline/probability by voice. Receipts now name people, companies and contacts instead of printing ids. |
