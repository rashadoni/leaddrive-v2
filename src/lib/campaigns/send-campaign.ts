...
/**
 * Campaign send logic, shared by the manual «Send Campaign» route
 * (`/api/v1/campaigns/[id]/send`) and the scheduled-send cron
 * (`src/lib/campaigns/scheduled-send-job.ts`).
 *
 * The caller owns tenancy (it must already be inside an RLS tenant scope for
 * `orgId`) and owns the claim that makes a send happen once: this module only
 * sends and writes the campaign's final status.
 */

type CampaignSendChannel = "email" | "sms" | "whatsapp" | "telegram"

/** HTTP-shaped outcome so the manual route can return it unchanged. */
export type CampaignSendResult = { status: number; body: Record<string, unknown> }

function sendResult(body: Record<string, unknown>, init?: { status: number }): CampaignSendResult {
...
type CampaignPrepareResult = { prepared: CampaignPreparedPayload } | { response: CampaignSendResult }
type CampaignRecipientResult = { recipients: CampaignRecipient[] } | { response: CampaignSendResult }
...
async function resolveSmsRecipients({ campaign, orgId }: CampaignSendContext): Promise<CampaignRecipientResult> {
  ...
  if (mode === "manual") {
    ...
    if (ids.length === 0) return { response: sendResult({ error: "No recipients selected" }, { status: 400 }) }
...
async function sendSmsCampaign({
  ...
  prepared,
  recipients,
}: CampaignSendContext & { prepared: CampaignPreparedPayload; recipients: CampaignRecipient[] }): Promise<CampaignSendResult> {
...
async function resolveEmailRecipients({ campaign, orgId }: CampaignSendContext): Promise<CampaignRecipientResult> {
  ...
  if (mode === "manual") {
    ...
    const ids = getRecipientIds(campaign.recipientIds)
    if (ids.length === 0) {
      return { response: sendResult({ error: "Не выбраны получатели" }, { status: 400 }) }
    }
    const recipients = await prisma.contact.findMany({
...
async function prepareTelegramCampaign({ orgId, campaign }: CampaignSendContext): Promise<CampaignPrepareResult> {
  ...
  if (!channel?.botToken) {
    return {
      response: sendResult({ error: "Telegram бот не настроен" }, { status: 422 }),
    }
  }
...
export type CampaignSendOutcome = CampaignSendResult & {
  /**
   * false — the campaign was refused before a single message left (channel not
   * configured, no recipients, a failure while loading them): its status was
   * NOT touched, and the caller decides what the campaign becomes.
   * true — the channel sender ran and wrote the final status itself
   * (sent / draft / ab_testing).
   */
  reachedSend: boolean
}

/**
 * Send one campaign. A throw from this function means it failed WHILE
 * sending — some recipients may already have their message — so a caller must
 * never retry it automatically.
 */
export async function sendCampaign(campaign: CampaignRecord, orgId: string): Promise<CampaignSendOutcome> {
  const adapter = getCampaignChannelAdapter(campaign)
  const ctx: CampaignSendContext = { campaign, campaignId: campaign.id, orgId }

  let prepared: CampaignPreparedPayload
  let recipients: CampaignRecipient[]
  try {
    const preparedResult = await adapter.prepare(ctx)
    if ("response" in preparedResult) return { ...preparedResult.response, reachedSend: false }

    const recipientResult = await adapter.resolveRecipients(ctx)
    if ("response" in recipientResult) return { ...recipientResult.response, reachedSend: false }

    prepared = preparedResult.prepared
    recipients = recipientResult.recipients.length > adapter.maxBatchSize
      ? recipientResult.recipients.slice(0, adapter.maxBatchSize)
      : recipientResult.recipients
  } catch (e) {
    console.error("[campaigns/send] failed before sending:", e)
    return { ...sendResult({ error: "Internal server error" }, { status: 500 }), reachedSend: false }
  }

  const result = await adapter.send({ ...ctx, prepared, recipients })
  return { ...result, reachedSend: true }
}
