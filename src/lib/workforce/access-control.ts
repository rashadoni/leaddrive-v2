...
const ROLE_PERMISSIONS: Readonly<Record<WorkforceAccessRole, readonly WorkforceAccessPermission[]>> = {
  ...
  // HR owns accountable exception resolution. A team manager may resolve
  // only inside an explicit team/site grant; neither role can turn a decision
  // into payroll or disciplinary action.
  HR_ADMIN: ["SCHEDULE_READ", "WORKFORCE_POLICY_DRAFT_WRITE", "WORKFORCE_EMPLOYMENT_MANAGE", "TEAM_ATTENDANCE_READ", "TEAM_REQUEST_READ", "TEAM_REQUEST_DECIDE", "TEAM_EXCEPTION_READ", "TEAM_EXCEPTION_DECIDE"],
  ...
  TEAM_MANAGER: ["TEAM_ATTENDANCE_READ", "TEAM_REQUEST_READ", "TEAM_REQUEST_DECIDE", "TEAM_EXCEPTION_READ", "TEAM_EXCEPTION_DECIDE"],
}
