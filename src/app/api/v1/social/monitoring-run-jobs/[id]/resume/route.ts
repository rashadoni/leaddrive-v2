...
export const POST = withSocialMonitoringMutationFence("social", "write", async (_request: NextRequest, auth, { params }: RouteCtx) => {
  ...
  try {
    const data = await resumeSocialMonitoringRunJob(auth.orgId, id, auth.userId, auth.role)
    ...
    console.error("[social-monitoring-run-jobs/resume] error", error)
    return NextResponse.json({ error: "social_monitoring_run_job_resume_failed" }, { status: 500 })
  }
})
