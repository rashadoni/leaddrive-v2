...
import { ContactSavedViewSchema, parseBody } from "@/lib/mtm-validators"
import { writeMtmAudit } from "@/lib/mtm-audit"

const ENTITY_TYPE = "mtm_contacts"
...
export const POST = withRouteFieldRlsAuth("read", async (req, auth) => {
  const parsed = parseBody(ContactSavedViewSchema, await req.json().catch(() => null))
  ...
  try {
    ...
    await writeMtmAudit({
      ...
      action: "CONTACT_VIEW_CREATE",
      entity: "saved_view",
      entityId: view.id,
      metadataKind: "contact_saved_view",
      newData: { name: view.name, isDefault: view.isDefault },
      req,
    }).catch((error) => console.warn("[MTM/contacts/views POST] audit failed", error))
    return NextResponse.json({ success: true, data: { view } }, { status: 201 })
  } catch (error) {
    console.error("[MTM/contacts/views POST]", error)
    return NextResponse.json({ error: "Failed to save contact view" }, { status: 500 })
  }
})
