...
import { OrganizationBulkAssignmentExecuteSchema, parseBody } from "@/lib/mtm-validators"
import {
  buildOrganizationAssignmentPreview,
  organizationAssignmentRequestHash,
} from "@/lib/mtm/organization-bulk-assignment"
...
function replay(operation: { requestHash: string; status: string; result: unknown }, requestHash: string) {
  if (operation.requestHash !== requestHash) {
    return NextResponse.json({ error: "Idempotency key was already used for another request", code: "MTM_ORGANIZATION_ASSIGNMENT_IDEMPOTENCY_MISMATCH" }, { status: 409 })
  }
  if (operation.status !== "COMPLETED" || !operation.result) {
    return NextResponse.json({ error: "Assignment is still being processed", code: "MTM_ORGANIZATION_ASSIGNMENT_IN_PROGRESS" }, { status: 409 })
  }
...
export const POST = withRouteFieldRlsAuth("write", async (req, auth) => {
  ...
  const parsed = parseBody(OrganizationBulkAssignmentExecuteSchema, await req.json().catch(() => null))
  ...
  const requestHash = organizationAssignmentRequestHash(body)
  const prior = await prisma.mtmOrganizationAssignmentOperation.findUnique({
  ...
  try {
    const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      await tx.mtmOrganizationAssignmentOperation.create({
        data: {
          organizationId: auth.orgId,
      ...
      const preview = await buildOrganizationAssignmentPreview(tx, {
        organizationId: auth.orgId,
      ...
      const changed: Array<{ organizationId: string; endedAssignmentIds: string[]; assignmentId: string | null }> = []
      for (const row of eligible) {
        const endedAssignmentIds: string[] = []
        for (const currentAssignmentId of row.currentAssignmentIds) {
          const ended = await tx.mtmCustomerAgentAssignment.updateMany({
            where: {
        ...
        if (body.mode === "ASSIGN" && body.targetAgentId) {
          const assignment = await tx.mtmCustomerAgentAssignment.create({
            data: {
              organizationId: auth.orgId,
              customerId: row.organizationId,
        ...
        changed.push({ organizationId: row.organizationId, endedAssignmentIds, assignmentId })
      }
      ...
      await tx.mtmOrganizationAssignmentOperation.update({
        where: { organizationId_idempotencyKey: { organizationId: auth.orgId, idempotencyKey: body.idempotencyKey } },
      ...
      await tx.mtmAuditLog.create({
        data: {
          ...
          action: body.mode === "ASSIGN" ? "ORGANIZATION_BULK_ASSIGN" : "ORGANIZATION_BULK_UNASSIGN",
          entity: "organization_assignment_batch",
          entityId: body.idempotencyKey,
          metadataKind: "organization_bulk_assignment",
          ...
          newData: {
            ...
            organizationIds: changed.map((item) => item.organizationId),
    ...
    if (error instanceof Error && error.message === "STALE_PREVIEW") {
      return NextResponse.json({ error: "The preview is stale; review the latest conflicts", code: "MTM_ORGANIZATION_ASSIGNMENT_STALE_PREVIEW" }, { status: 409 })
    }
    if (error instanceof Error && error.message === "NOTHING_TO_ASSIGN") {
      return NextResponse.json({ error: "No selected organizations can be changed", code: "MTM_ORGANIZATION_ASSIGNMENT_EMPTY" }, { status: 409 })
    }
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
      const concurrent = await prisma.mtmOrganizationAssignmentOperation.findUnique({
        where: { organizationId_idempotencyKey: { organizationId: auth.orgId, idempotencyKey: body.idempotencyKey } },
        select: { requestHash: true, status: true, result: true },
      })
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2034") {
      return NextResponse.json({ error: "Assignments changed concurrently; refresh the preview", code: "MTM_ORGANIZATION_ASSIGNMENT_CONFLICT" }, { status: 409 })
    }
    console.error("[MTM/organization-assignments POST]", error)
    return NextResponse.json({ error: "Failed to update organization assignments" }, { status: 500 })
  }
})
