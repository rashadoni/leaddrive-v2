/**
 * CLM Slice 7a — Teams ChannelConfig CRUD.
 *
 * GET    /api/v1/integrations/teams          — list org's Teams configs
 * POST   /api/v1/integrations/teams          — create a Teams config OR test a webhook URL
 * PUT    /api/v1/integrations/teams          — update a Teams config (inc. contractAlerts toggle)
 * DELETE /api/v1/integrations/teams?id=…    — delete a Teams config
 *
 * Mirrors the Slack route exactly, using channelType "teams".
 * The `settings.contractAlerts` boolean is the per-config toggle that gates
 * sendContractAlert() for this webhook.
 */
...
export const GET = withRlsAuth("contracts", "read", async (_req, session) => {
  ...
  const configs = await prisma.channelConfig.findMany({
    where: { organizationId: orgId, channelType: "teams" },
...
export const POST = withRlsAuth("contracts", "write", async (req, session) => {
  ...
  // Test action — send a test card without persisting anything
  if (body.action === "test" && body.webhookUrl) {
    // FIX 1: validate before sending
    try {
      assertSafeWebhookUrl(body.webhookUrl, "teams")
    } catch (err) {
      return NextResponse.json({ error: (err as Error).message }, { status: 400 })
    }
    const success = await sendTeamsNotification(body.webhookUrl, {
      summary: "LeadDrive CRM test message — Teams integration is working!",
      sections: [
        {
  ...
  try {
    assertSafeWebhookUrl(parsed.data.webhookUrl, "teams")
  } catch (err) {
    return NextResponse.json({ error: (err as Error).message }, { status: 400 })
  }
...
export const PUT = withRlsAuth("contracts", "write", async (req, session) => {
  ...
  const parsed = teamsPutSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.issues[0].message }, { status: 400 })
  }
  ...
  const result = await prisma.channelConfig.updateMany({
    where: { id, organizationId: orgId, channelType: "teams" },
    data,
  })
