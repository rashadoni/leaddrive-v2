...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, init?: { method?: string; body?: string }) => {
    ...
    if (url.endsWith("/phone/telegram")) return new Response(JSON.stringify({ success: true, verified, linkOpen: !verified }))
...
describe("proving the phone through Telegram", () => {
  ...
  it("gives a real link to tap, waits for the bot, and moves on by itself", async () => {
    ...
    expect(requests[0]).toMatchObject({ method: "POST", body: { consent: true } })
    expect(JSON.stringify(requests[0].body)).not.toMatch(/\d{9}/)
  ...
  it("keeps the SMS code one tap away", async () => {
    ...
    expect(requests[0]).toMatchObject({ method: "POST", body: { useRequestPhone: true } })
