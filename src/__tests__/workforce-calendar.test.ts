...
describe("Workforce calendar semantics", () => {
  it("separates an ordinary scheduled day, non-working weekend and public holiday", () => {
    expect(
      resolveWorkforceCalendarDay({ date: "2026-09-01", overrides: [] }),
    ).toMatchObject({
      state: "SCHEDULED",
    ...
    expect(
      resolveWorkforceCalendarDay({ date: "2026-09-05", overrides: [] }),
    ).toMatchObject({
      state: "NON_WORKING",
  ...
  it("recognizes only explicit Workforce request sources as approved leave or absence", () => {
    ...
    expect(
      resolveWorkforceCalendarDay({
        date: DATE,
        agentId: "agent-1",
        overrides: [{ ...base, source: "WORKFORCE_LEAVE" }],
      }),
    ).toMatchObject({
    ...
    expect(
      resolveWorkforceCalendarDay({
        date: DATE,
        agentId: "agent-1",
        overrides: [{ ...base, source: "WORKFORCE_ABSENCE" }],
      }),
    ).toMatchObject({
    ...
    expect(
      resolveWorkforceCalendarDay({
        date: DATE,
        agentId: "agent-1",
        overrides: [{ ...base, source: "HRM" }],
      }),
    ).toMatchObject({
      state: "PERSONAL_EXCEPTION",
      excused: true,
      noShowEligible: false,
    });
  });
  ...
  it("loads only the employee, team and organization candidates before resolving precedence", async () => {
    ...
    await expect(
      resolvePersistedWorkforceCalendarDay(prisma as never, {
        organizationId: "org-1",
        agentId: "agent-1",
        date: DATE,
      }),
    ).resolves.toMatchObject({
      state: "TENANT_CLOSURE",
      attendanceExpected: false,
    });
  ...
  it("uses the historical team rather than a later directory transfer for a past calendar day", async () => {
    ...
    await expect(
      resolveHistoricalPersistedWorkforceCalendarDay(prisma as never, {
        organizationId: "org-1",
        agentId: "agent-1",
        date: DATE,
    ...
    const calendarQuery = vi
      .mocked(prisma.mtmWorkCalendarDay.findMany)
      .mock.calls.at(-1)?.[0];
    expect(calendarQuery).toMatchObject({
      where: expect.objectContaining({
        OR: expect.arrayContaining([{ agentId: null, teamId: "team-history" }]),
      }),
    });
  ...
  it("fails closed for impossible dates and unavailable employees", async () => {
    expect(() =>
      resolveWorkforceCalendarDay({ date: "2026-02-30", overrides: [] }),
    ).toThrowError(WorkforceCalendarResolutionError);
