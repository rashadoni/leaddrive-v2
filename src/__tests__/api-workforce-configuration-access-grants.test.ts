...
describe("Workforce access-grant configuration API", () => {
  ...
  it("returns the first grant receipt when the same HTTP operation is retried", async () => {
    vi.mocked(prisma.workforceAccessGrant.create).mockRejectedValueOnce({ code: "P2002" } as never)
  ...
  it("does not consume a role-management rate budget before input/self/bootstrap safety checks", async () => {
  ...
  it("fails closed before grant target, ledger or audit access when the shared rate guard denies", async () => {
    ...
    expect(requireWorkforceAccessGrantRateLimit).toHaveBeenCalledWith({
      operation: "MUTATION", organizationId: auth.orgId, principalUserId: auth.userId,
    })
    expect(prisma.user.findFirst).not.toHaveBeenCalled()
    expect(prisma.workforceAccessGrant.findMany).not.toHaveBeenCalled()
    expect(prisma.mtmAuditLog.create).not.toHaveBeenCalled()
  ...
  it("rechecks the active tenant target inside the serializable transaction", async () => {
    vi.mocked(prisma.user.findFirst)
      .mockResolvedValueOnce({ id: "employee_2" } as never)
  ...
  it("returns the first revocation receipt when the same HTTP operation is retried", async () => {
    vi.mocked(prisma.workforceAccessGrant.findFirst)
      .mockResolvedValueOnce(revocableGrant as never)
      .mockResolvedValueOnce(revocableGrant as never)
    vi.mocked(prisma.workforceAccessGrantRevocation.create).mockRejectedValueOnce({ code: "P2002" } as never)
    vi.mocked(prisma.workforceAccessGrantRevocation.findFirst).mockResolvedValueOnce({
      id: "revocation_new_1",
      organizationId: auth.orgId,
      grantId: revocableGrant.id,
      operationId: "revoke-op-1",
      revokedByUserId: auth.userId,
      revocationReasonCode: "ROLE_CHANGE",
      revokedAt: new Date("2026-09-10T08:00:00.000Z"),
    } as never)
  ...
  it("blocks a validated revocation before any grant lookup when the shared rate guard denies", async () => {
    vi.mocked(requireWorkforceAccessGrantRateLimit).mockResolvedValue(NextResponse.json({
