...
import { GET as previewApproval } from "@/app/api/v1/workforce/timesheet/approvals/[id]/preview/route"
...
const AUTH = { orgId: "org-workforce", userId: "custodian-1", role: "manager", principalType: "session" as const }
type RouteContext = { params: Promise<{ id: string }> }
const invoke = previewApproval as unknown as (
  request: NextRequest,
  auth: typeof AUTH,
...
function request(purpose = "HR_RECORD_REVIEW") {
  return new NextRequest(
    `http://localhost:3000/api/v1/workforce/timesheet/approvals/approval-1/preview?purpose=${purpose}`,
    { headers: { "user-agent": "vitest-preview" } },
  )
}
...
describe("GET /api/v1/workforce/timesheet/approvals/:id/preview", () => {
  it("returns a verified immutable preview and appends metadata-only audit evidence", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      success: true,
      data: {
        approval: { id: "approval-1", recordKind: "CORRECTION", revision: 2, calculationVersion: 1 },
        scope: {
        ...
        rows: [{ workdayId: "day-1", workedSeconds: 28800, overtimeClassification: "OPERATIONAL_DEVIATION_NOT_PAYABLE" }],
      },
    })
    ...
    expect(JSON.stringify(audit.data.newData)).not.toContain("workdayId")
  })

  it("requires MFA and validates identifier and fixed purpose before charging or reading", async () => {
  ...
  it("stops before lookup and audit when the shared limiter denies", async () => {
  ...
  it("fails closed for missing, disappeared and hash-inconsistent approvals", async () => {
  ...
  it("contains unexpected failures without reflecting sensitive details", async () => {
    const privateFailure = new Error("raw private preview failure employee-42")
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      { operation: "preview-timesheet-approval-export" },
    )
