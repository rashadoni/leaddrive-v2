...
describe("Workforce employee exception-response shared rate guard", () => {
  it("uses an opaque Redis-partitioned self-response budget", async () => {
    vi.mocked(consumePublicRateLimitBatch).mockResolvedValue({
      allowed: true, unavailable: false, retryAfterSeconds: 0,
    })

    await expect(requireWorkforceExceptionEmployeeResponseRateLimit({
      organizationId: "org-1", principalUserId: "employee-1",
    })).resolves.toBeNull()

    expect(hashForRateLimit).toHaveBeenCalledWith(
      "workforce-exception-employee-response-partition:v1:org-1",
    )
    expect(consumePublicRateLimitBatch).toHaveBeenCalledWith([expect.objectContaining({
      ...
      identifier: "org-1:employee-1",
      identifierMode: "exact",
      redisHashTag: "workforce-exception-employee-response:partition-hash",
  ...
  it("returns bounded no-store denial or availability responses", async () => {
    ...
    const denied = await requireWorkforceExceptionEmployeeResponseRateLimit({
      organizationId: "org-1", principalUserId: "employee-1",
    })
    const unavailable = await requireWorkforceExceptionEmployeeResponseRateLimit({
      organizationId: "org-1", principalUserId: "employee-1",
    })
    ...
    await expect(denied?.json()).resolves.toMatchObject({
      code: "WORKFORCE_EXCEPTION_EMPLOYEE_RESPONSE_RATE_LIMITED",
    })
  ...
  it("fails closed if hashing or the shared guard raises", async () => {
    ...
    const response = await requireWorkforceExceptionEmployeeResponseRateLimit({
      organizationId: "org-1", principalUserId: "employee-1",
    })
