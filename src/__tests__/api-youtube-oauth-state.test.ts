...
/**
 * YouTube OAuth callback — signed state carried in the `state` param (survives any host) OR the cookie
 * (same-host). Port of the TikTok subdomain fix (#257): a cookie-only state caused `missing_cookie`
 * on tenant subdomains where the cookie host ≠ the fixed YOUTUBE_REDIRECT_URI host. The HMAC signature
 * guards integrity; the cookie↔state cross-check guards CSRF when both are present; the param-only
 * path additionally requires a matching session.
 */
vi.mock("@/lib/prisma", () => ({
...
import { GET } from "@/app/api/v1/social/oauth/youtube/callback/route"
...
function req(stateParam: string, cookieVal?: string): NextRequest {
  return {
    url: `http://0.0.0.0:3001/api/v1/social/oauth/youtube/callback?code=CODE&state=${encodeURIComponent(stateParam)}`,
    cookies: { get: (n: string) => (n === "ld_yt_oauth" && cookieVal ? { value: cookieVal } : undefined) },
    headers: {
      get: (h: string) =>
...
beforeEach(() => {
  vi.clearAllMocks()
  delete process.env.GOOGLE_CLIENT_ID // valid state then stops at "not_configured" — got PAST the state gate
...
describe("YouTube OAuth callback signed-state acceptance", () => {
