-- COMPLETE fix-forward for 20260518120000_health (R2 Health Cloud).
--
-- The original migration had a forward-FK ordering bug — FK was declared
-- BEFORE the parent table existed. Since Prisma wraps each migration in
-- a transaction, ALL DDL was rolled back. NO health tables exist on prod.
--
-- This is a COMPLETE idempotent rewrite that creates all 5 tables
-- (providers, patients, medical_records, encounters, care_plans), all
-- indexes, all triggers, all constraints, AND the previously-broken
-- FK at the END (after health_encounters exists).
--
...
-- ALTER TABLE ADD CONSTRAINT wrapped in DO block with pg_constraint check.
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_providers_role_check') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_providers_npi_check') THEN
    ALTER TABLE "health_providers"
...
CREATE UNIQUE INDEX IF NOT EXISTS "health_providers_org_email_uniq"
  ON "health_providers"("organizationId", "email");
CREATE INDEX IF NOT EXISTS "health_providers_org_active_idx"
  ON "health_providers"("organizationId", "isActive");
CREATE INDEX IF NOT EXISTS "health_providers_org_role_idx"
  ON "health_providers"("organizationId", "role");
CREATE INDEX IF NOT EXISTS "health_providers_specialty_idx"
  ON "health_providers"("organizationId", "specialty");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_providers_organizationId_fkey') THEN
    ALTER TABLE "health_providers"
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_status_check') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_mrn_check') THEN
    ALTER TABLE "health_patients"
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_discharged_coherence_check') THEN
    ALTER TABLE "health_patients"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_deceased_coherence_check') THEN
    ALTER TABLE "health_patients"
...
CREATE UNIQUE INDEX IF NOT EXISTS "health_patients_org_mrn_uniq"
  ON "health_patients"("organizationId", "mrn");
CREATE INDEX IF NOT EXISTS "health_patients_org_status_idx"
  ON "health_patients"("organizationId", "status");
CREATE INDEX IF NOT EXISTS "health_patients_primary_provider_idx"
  ON "health_patients"("primaryProviderId");
CREATE INDEX IF NOT EXISTS "health_patients_contact_idx"
  ON "health_patients"("contactId");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_organizationId_fkey') THEN
    ALTER TABLE "health_patients"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_patients_primaryProviderId_fkey') THEN
    ALTER TABLE "health_patients"
...
DROP TRIGGER IF EXISTS health_patients_timestamps_immutable_trigger ON "health_patients";
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_type_check') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_severity_check') THEN
    ALTER TABLE "health_medical_records"
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_sensitivity_check') THEN
    ALTER TABLE "health_medical_records"
...
CREATE INDEX IF NOT EXISTS "health_medical_records_patient_recorded_idx"
  ON "health_medical_records"("patientId", "recordedAt");
CREATE INDEX IF NOT EXISTS "health_medical_records_org_type_idx"
  ON "health_medical_records"("organizationId", "recordType");
CREATE INDEX IF NOT EXISTS "health_medical_records_encounter_idx"
  ON "health_medical_records"("encounterId");
CREATE INDEX IF NOT EXISTS "health_medical_records_provider_idx"
  ON "health_medical_records"("recordedByProviderId");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_organizationId_fkey') THEN
    ALTER TABLE "health_medical_records"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_patientId_fkey') THEN
    ALTER TABLE "health_medical_records"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_recordedByProviderId_fkey') THEN
    ALTER TABLE "health_medical_records"
...
DROP TRIGGER IF EXISTS health_medical_records_no_update_trigger ON "health_medical_records";
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_type_check') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_status_check') THEN
    ALTER TABLE "health_encounters"
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_time_check') THEN
    ALTER TABLE "health_encounters"
...
-- Status-timestamp coherence
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_checked_in_coherence_check') THEN
    ALTER TABLE "health_encounters"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_started_coherence_check') THEN
    ALTER TABLE "health_encounters"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_completed_coherence_check') THEN
    ALTER TABLE "health_encounters"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_cancelled_coherence_check') THEN
    ALTER TABLE "health_encounters"
...
CREATE INDEX IF NOT EXISTS "health_encounters_patient_scheduled_idx"
  ON "health_encounters"("patientId", "scheduledStartAt");
CREATE INDEX IF NOT EXISTS "health_encounters_provider_scheduled_idx"
  ON "health_encounters"("providerId", "scheduledStartAt");
CREATE INDEX IF NOT EXISTS "health_encounters_org_status_idx"
  ON "health_encounters"("organizationId", "status");
CREATE INDEX IF NOT EXISTS "health_encounters_org_type_idx"
  ON "health_encounters"("organizationId", "encounterType");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_organizationId_fkey') THEN
    ALTER TABLE "health_encounters"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_patientId_fkey') THEN
    ALTER TABLE "health_encounters"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_encounters_providerId_fkey') THEN
    ALTER TABLE "health_encounters"
...
DROP TRIGGER IF EXISTS health_encounters_timestamps_immutable_trigger ON "health_encounters";
...
CREATE TABLE IF NOT EXISTS "health_care_plans" (
    "id" TEXT NOT NULL,
    "organizationId" TEXT NOT NULL,
    "patientId" TEXT NOT NULL,
    "name" TEXT NOT NULL,
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_status_check') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_period_check') THEN
    ALTER TABLE "health_care_plans"
...
-- Status-timestamp coherence
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_active_coherence_check') THEN
    ALTER TABLE "health_care_plans"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_completed_coherence_check') THEN
    ALTER TABLE "health_care_plans"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_cancelled_coherence_check') THEN
    ALTER TABLE "health_care_plans"
...
CREATE INDEX IF NOT EXISTS "health_care_plans_patient_status_idx"
  ON "health_care_plans"("patientId", "status");
CREATE INDEX IF NOT EXISTS "health_care_plans_org_status_idx"
  ON "health_care_plans"("organizationId", "status");
CREATE INDEX IF NOT EXISTS "health_care_plans_provider_idx"
  ON "health_care_plans"("providerId");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_organizationId_fkey') THEN
    ALTER TABLE "health_care_plans"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_patientId_fkey') THEN
    ALTER TABLE "health_care_plans"
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_care_plans_providerId_fkey') THEN
    ALTER TABLE "health_care_plans"
...
DROP TRIGGER IF EXISTS health_care_plans_timestamps_immutable_trigger ON "health_care_plans";
...
-- ── The missing FK from the original failure point ─────────────
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'health_medical_records_encounterId_fkey') THEN
    ALTER TABLE "health_medical_records"
      ADD CONSTRAINT "health_medical_records_encounterId_fkey"
      FOREIGN KEY ("encounterId") REFERENCES "health_encounters"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END $$;
