...
probe_current_backup_database_state() {
  ...
  for key in PGHOST PGHOSTADDR PGPORT PGDATABASE PGUSER PGPASSFILE PGSSLMODE PGSSLROOTCERT PGCONNECT_TIMEOUT; do
    value="${CONFIG[$key]:-}"
...
config_keys=(
  PGHOST PGHOSTADDR PGPORT PGDATABASE PGUSER PGPASSFILE PGSSLMODE PGSSLROOTCERT PGCONNECT_TIMEOUT
...
required_keys=(
  PGHOST PGHOSTADDR PGDATABASE PGUSER PGPASSFILE PGSSLROOTCERT BACKUP_EXPECTED_DB_ROLE
...
if [[ "${CONFIG[PGHOSTADDR]:-}" =~ ^[0-9A-Fa-f:.]+$ ]]; then
  pass "source certificate identity uses a separate numeric route address"
else
  fail "PGHOSTADDR must be a numeric PostgreSQL route address"
fi
if [ -n "${CONFIG[PGHOSTADDR]:-}" ] && [ -n "${CONFIG[VERIFY_PGHOST]:-}" ] \
  && [ "${CONFIG[PGHOSTADDR]}:${source_port}" != "${CONFIG[VERIFY_PGHOST]}:${CONFIG[VERIFY_PGPORT]:-}" ]; then
